...
{
    private static final Logger logUtility = LogManager.getLogger(MainApp.class);
	...
	while(true)
	{
		...
		else if(choice == 1)
		{
			...
			Double result = calcObj.findSquareRoot(inpX);
		...
		else if(choice == 2)
		{
			...
			Integer result = calcObj.findFactorial(inpX);
		...
		else if(choice == 3)
		{
			...
			double inpX = inputReceiver.nextDouble();
			Double result = calcObj.findNaturalLog(inpX);
		...
		else
		{
			...
			Double result = calcObj.findPower(base, power);
